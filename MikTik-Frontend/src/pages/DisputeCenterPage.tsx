...
const STATUS_CFG = {
  open:     { label: 'Open',     icon: AlertTriangle, classes: 'bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800' },
  resolved: { label: 'Resolved', icon: CheckCircle,   classes: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800' },
  pending:  { label: 'Pending',  icon: Clock,         classes: 'bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-white/10' },
};
    ...
    <div className="bg-white dark:bg-zinc-950 min-h-screen">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Dispute Center</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Raise and track issues with your transactions</p>
          </div>
          <button
        ...
        <div className="flex items-start gap-3 bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-4 mb-6">
          ...
          <div>
            <p className="text-sm font-semibold text-indigo-900 dark:text-indigo-300 mb-0.5">Buyer protection active</p>
            <p className="text-xs text-indigo-700 dark:text-indigo-400 leading-relaxed">
              All payments are held in escrow. If you have an issue, open a dispute and our team will mediate within 24 hours.
            </p>
          </div>
        </div>
        ...
        {showNew && (
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5 mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-slate-900 dark:text-white text-sm">Open a new dispute</h2>
            ...
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Order ID</label>
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">Reason</label>
                <textarea
                  value={reason}
        ...
        <div className="space-y-3">
          {DISPUTES.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-slate-500 dark:text-slate-400 font-medium">No disputes yet</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">All your transactions are running smoothly.</p>
            </div>
          ) : (
            DISPUTES.map(d => {
                ...
                <div key={d.id} className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-white/5 rounded-2xl p-5">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{d.id} · Opened {d.opened}</p>
                    </div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-white/5">
                    <span className="text-xs text-slate-400 dark:text-slate-500">Order: {d.order} · ₪{d.amount}</span>
                    {d.status === 'open' && (
                    ...
                    {d.status === 'resolved' && (
                      <span className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Refund issued</span>
                    )}
                  </div>
                </div>
              );
