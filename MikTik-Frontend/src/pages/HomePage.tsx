...
const CAT_TAG: Record<string, string> = {
  Concert: "bg-purple-500/80",
  Sports: "bg-emerald-500/80",
  Theater: "bg-rose-500/80",
  Festival: "bg-orange-500/80",
...
const CATEGORY_DATA = [
  {
    key: "sports",
    emoji: "⚽",
    to: "/marketplace?category=sports",
    color:
      "from-emerald-900/60 to-teal-950/60 border-emerald-800/40 hover:border-emerald-600/60",
  },
  {
    key: "concerts",
    emoji: "🎵",
    to: "/marketplace?category=concert",
    color:
      "from-violet-900/60 to-indigo-950/60 border-violet-800/40 hover:border-violet-600/60",
  },
  {
    key: "theater",
    emoji: "🎭",
    to: "/marketplace?category=theater",
    color:
      "from-rose-900/60 to-pink-950/60 border-rose-800/40 hover:border-rose-600/60",
  },
  {
    key: "festivals",
    emoji: "🎪",
    to: "/marketplace?category=festival",
    color:
      "from-orange-900/60 to-amber-950/60 border-orange-800/40 hover:border-orange-600/60",
  },
  {
    key: "comedy",
    emoji: "😂",
    to: "/marketplace?category=comedy",
    color:
      "from-yellow-900/60 to-amber-950/60 border-yellow-800/40 hover:border-yellow-600/60",
  },
  {
    key: "standup",
    emoji: "🎤",
    to: "/marketplace?category=comedy",
    color:
      "from-sky-900/60 to-blue-950/60 border-sky-800/40 hover:border-sky-600/60",
  },
];
...
export default function HomePage() {
  ...
  const FEATURES = [
    {
      icon: ShieldCheck,
      title: t("feature.escrow.title"),
      desc: t("feature.escrow.desc"),
    },
    {
      icon: Lock,
      title: t("feature.sellers.title"),
      desc: t("feature.sellers.desc"),
    },
    {
      icon: Zap,
      title: t("feature.transfer.title"),
      desc: t("feature.transfer.desc"),
    },
    {
      icon: CreditCard,
      title: t("feature.payments.title"),
      desc: t("feature.payments.desc"),
    },
    {
      icon: Users,
      title: t("feature.dispute.title"),
      desc: t("feature.dispute.desc"),
    },
    {
      icon: Star,
      title: t("feature.ratings.title"),
      desc: t("feature.ratings.desc"),
    },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {featuredTickets.map((e) => (
            <Link
              key={e.id}
              to={`/ticket/${e.id}`}
              className="group block rounded-2xl overflow-hidden border border-slate-200 dark:border-white/5 hover:border-indigo-300 dark:hover:border-indigo-500/40 transition-all duration-300 hover:shadow-xl dark:hover:shadow-black/50 hover:-translate-y-1 bg-white dark:bg-zinc-900"
            >
              <div
                className={`relative h-44 bg-linear-to-br ${CAT_GRADIENT[e.category] || CAT_GRADIENT["Concert"]}`}
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-3.5">
                  <span
                    className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md text-white mb-1.5 inline-block ${CAT_TAG[e.category] || ""}`}
                  >
                    {e.category}
                  </span>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {STEPS.map((s, i) => (
              <div
                key={s.n}
                className="relative bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-white/5 p-8"
              >
                {i < STEPS.length - 1 && (
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="group p-6 rounded-2xl border border-slate-200 dark:border-white/5 hover:border-indigo-200 dark:hover:border-indigo-500/30 bg-white dark:bg-zinc-900 transition-all hover:-translate-y-0.5 hover:shadow-lg dark:hover:shadow-black/30"
            >
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="relative overflow-hidden rounded-3xl bg-linear-to-br from-indigo-600 via-indigo-700 to-violet-700 dark:from-indigo-950 dark:via-indigo-900 dark:to-violet-950 border border-indigo-500/30 px-8 py-16 text-center">
          ...
          <div className="relative">
            ...
            <p className="text-indigo-200 dark:text-indigo-300 text-sm mb-8 max-w-md mx-auto">
              {t("cta.sub")}
            </p>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Link
                to="/register"
                className="px-6 py-3 rounded-xl bg-white hover:bg-indigo-50 text-indigo-700 font-semibold text-sm transition-colors shadow-lg"
              >
                {t("cta.createAccount")}
              </Link>
              <Link
