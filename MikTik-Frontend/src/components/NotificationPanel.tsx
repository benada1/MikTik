...
export default function NotificationPanel({ onClose }: Props) {
  ...
  return (
    <div
      ...
      aria-modal="true"
      className="absolute right-0 top-full mt-2 w-80 bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 z-50 overflow-hidden"
    >
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-white/8">
        <div className="flex items-center gap-2">
          <Bell className="w-4 h-4 text-slate-600 dark:text-slate-400" aria-hidden="true" />
          <span className="font-semibold text-sm text-slate-900 dark:text-white">
            {t('notif.title')}
          </span>
          {unreadCount > 0 && (
            <span className="bg-indigo-600 text-white text-[10px] font-bold rounded-full px-1.5 py-0.5 leading-none" aria-hidden="true">
              {unreadCount}
            </span>
          )}
        </div>
      ...
      {/* List */}
      <div className="max-h-96 overflow-y-auto scrollbar-themed" aria-live="polite" aria-relevant="additions removals">
          ...
          notifications.map(n => (
            <div key={n._id} className="group relative">
              <button
                ...
                className={`w-full text-left flex items-start gap-3 px-4 py-3 border-b border-slate-50 dark:border-white/5 transition-colors hover:bg-slate-50 dark:hover:bg-white/5 focus:outline-none focus:bg-slate-50 dark:focus:bg-white/5 ${
              ...
              >
                ...
                <div className="flex-1 min-w-0 pr-6">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-[10px] text-slate-600 dark:text-slate-400">{timeAgo(n.createdAt)}</span>
                    {!n.read && <span className="w-1.5 h-1.5 rounded-full bg-indigo-500 dark:bg-indigo-400 shrink-0" aria-label="Unread" />}
                  </div>
                </div>
              </button>
              <button
                onClick={e => { e.stopPropagation(); deleteNotification(n._id); }}
                className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 p-0.5 rounded hover:bg-slate-200 dark:hover:bg-white/15 transition-all"
